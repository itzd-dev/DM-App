...
export const UserProvider = ({ children }) => {
  ...
  const [customerPoints, setCustomerPoints] = useState(() =>
    safeLoad("customerPoints", {})
  );
  const [customerProfiles, setCustomerProfiles] = useState(() =>
    safeLoad("customerProfiles", {})
  );
  ...
  useEffect(
    () => safeSave("customerProfiles", customerProfiles),
    [customerProfiles]
  );
  ...
  const fetchUserState = useCallback(async () => {
    ...
    try {
      ...
      const profilesData =
        data && typeof data.profiles === "object" && data.profiles !== null
          ? data.profiles
          : {};
    ...
    async (partial) => {
      ...
      try {
        ...
        if (partial.profiles && partial.profiles[loggedInUser.email]) {
          try {
            lastPersistedProfile.current = JSON.stringify(
              partial.profiles[loggedInUser.email]
            );
          } catch (_) {
            lastPersistedProfile.current = null;
          }
  ...
  const fetchAllCustomerPoints = useCallback(async () => {
    try {
      console.log("[UserContext] Fetching all customer points...");
      const headers = await getAuthHeaders();
      const resp = await fetch("/api/loyalty?all=true", { headers });
      console.log("[UserContext] API Response status:", resp.status);
      ...
      if (resp.ok) {
        ...
        if (typeof data === "object" && data !== null) {
          ...
          console.log("[UserContext] Customer points updated:", data);
        } else {
          console.warn("[UserContext] Invalid data format:", data);
        }
      } else {
        const errorText = await resp.text();
        console.error("[UserContext] API Error:", resp.status, errorText);
      }
    } catch (error) {
      console.error("[UserContext] fetchAllCustomerPoints failed", error);
    }
  }, [getAuthHeaders]);
